...
function addAdultAPI() {
    ...
    if (!HIDE_BUILTIN_ADULT_APIS && (localStorage.getItem('yellowFilterEnabled') === 'false')) {
        ...
        adultTitle.innerHTML = `18+资源采集站 <span class="adult-warning">
...
function checkAdultAPIsSelected() {
    ...
    if (hasAdultSelected) {
        ...
        if (filterDescription) {
            filterDescription.innerHTML = '<strong class="text-pink-300">选中18+资源站时无法启用此过滤</strong>';
        }
        ...
        // 恢复原来的描述文字
        if (filterDescription) {
            filterDescription.innerHTML = '过滤"伦理片"等18+内容';
        }
...
async function search() {
    ...
    try {
        ...
        // 处理搜索结果过滤：如果启用了18+内容过滤，则过滤掉分类含有敏感内容的项目
